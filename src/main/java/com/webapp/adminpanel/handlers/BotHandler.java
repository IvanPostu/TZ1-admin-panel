...
public class BotHandler {
  ...
  public Mono<ServerResponse> findBotsByName(ServerRequest request) {
    ...
    final int botsPerPage = request.queryParam("bots-per-page").map(a -> {
      int result = Integer.parseInt(a);
      return Math.min(result, 100);
    }).orElse(7);
    final int currentPage = request.queryParam("current-page").map(Integer::valueOf).orElse(0);
    PageRequest pageRequest = PageRequest.of(currentPage, botsPerPage);

    Mono<FindBotsResultDto> data = Mono.just(botService.findBotByNameSubstring(botName, pageRequest)).map(a -> {
    ...
    return ServerResponse.ok().hint(Jackson2CodecSupport.JSON_VIEW_HINT, FindBotsResultDtoViews.FindBotsDtoData.class)
        .contentType(MediaType.APPLICATION_JSON).body(data, FindBotsResultDto.class);
  ...
  public Mono<ServerResponse> findBotById(ServerRequest request) {
    ...
    Mono<BotDto> result = Mono.just(botService.findById(botId)).map(a -> {
      BotDto res = new BotDto();
      BotEntity botFromDb = a.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Bot not found!"));
    ...
    return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(result, BotDto.class);
  }
  ...
  public Mono<ServerResponse> botSubscribers(ServerRequest request) {
    ...
    try {
      Mono<BotSubscribersDto> result = Mono.just(botService.findSubscribersByBotId(botId)).map(a -> {
        BotSubscribersDto r = new BotSubscribersDto();
        r.setSubscribers(a);
        r.setBotId(botId);
        return r;
      });
      return ServerResponse.ok().hint(Jackson2CodecSupport.JSON_VIEW_HINT, BotSubscribersDtoViews.All.class)
          .contentType(MediaType.APPLICATION_JSON).body(result, BotSubscribersDto.class);
    } catch (Exception e) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Id is not valid!");
    }
  }
